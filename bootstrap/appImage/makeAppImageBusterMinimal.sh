#!/bin/bash
# AppImage builder for Buster (Minimal)
# Requires environment to be prepared via prepare_env_buster.sh
...
# Build
bash bootstrap/scripts/linux.sh --with-system-libmad ${rebuild} 2>&1 | tee ${logfile}
if [ ${PIPESTATUS[0]} -ne 0 ]; then
    fail "Build failed, please inspect ${logfile} and /tmp/logbuild* files."
fi

# Package
bash bootstrap/appImage/deployBusterMinimal.sh "${PWD}/${RT_DIR}/${RUNTIME}"
exit $?
